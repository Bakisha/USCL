...
Animations are written in as sketch in Arduino IDE, with each animation code per tab, and only 1 line of code is to select how cube is built (see examples). To select cpu, just select proper board in Arduino IDE options. 
...
MCU sends data via SPI hardware pins, and two other (selectable) pins is used for Output Enable and Latch pin for 74hc595.
 ...
 Generaly, it need 'Y size' pins , multiplied by 'X size', divided by 8 (rounded up), and multipled by 3 (if using RGB cube), plus one UDN2981 (or similar hardware) per every 8 layers of cube
...
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  OE pin          = PA0  // user defined                                                 = D10  // user defined            = A1  // User defined           = 49  // User defined           = D0  IO - user defined - OR D4  IO, 10k Pull-up, BUILTIN_LED - user defined
  Latch pin       = PA1  // user defined                                                 = D9   // user defined            = A0  // User defined           = 47  // User defined           = D8  IO, 10k Pull-down, SS    - user defined
...
It means , library will keep showing front frame buffer, while user draw in back buffer, then use command (  cube.drawVoxels() )  to transfer back buffer to front buffer.
This gives very nice non-flickering experience. 
...
-STM32F1 (Bluepill, Blackpill)
 ...
 or set maximum spped of your hardware as:
 SPI_speed = 12000000 // set maximimum speed of 595 chip. First lower speed compatible with CPU SPI hardware will be selected
...
You can write animations in main loop, but i recommend you create functions and pass paramaters. So it can be nice list of animation names (or write extra code to random select animation). I didn't bother to write animations as library. But i managed to write it as animation per tab in Arduino IDE. In last tab is list of animations. Unfortunately, that's how Arduino IDE manage tabs, it can call only functions in previous tabs, so list must be in last tab.
